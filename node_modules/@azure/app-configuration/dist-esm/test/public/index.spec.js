...
import { assertEqualSettings, assertThrowsAbortError, assertThrowsRestError, createAppConfigurationClientForTests, deleteEverySetting, deleteKeyCompletely, startRecorder, toSortedArray, } from "./utils/testHelpers";
...
describe("AppConfigurationClient", () => {
    ...
    describe("addConfigurationSetting", () => {
        ...
        it("throws an error if the configuration setting already exists", async () => {
            ...
            try {
                ...
                assert.equal(err.message, "Status 412: Setting was already present");
    ...
    describe("listConfigurationSettings", () => {
        ...
        beforeEach(async () => {
            keys.listConfigSettingA = recorder.variable(`listConfigSetting${count}A`, `listConfigSetting${count}A${Math.floor(Math.random() * 100000)}`);
            keys.listConfigSettingB = recorder.variable(`listConfigSetting${count}B`, `listConfigSetting${count}B${Math.floor(Math.random() * 100000)}`);
            count += 1;
            uniqueLabel = recorder.variable("listConfigSettingsLabel", `listConfigSettingsLabel${Math.floor(Math.random() * 100000)}`);
        ...
        it("list with multiple pages - bypage and etags", async function () {
            ...
            async function addConfigSettings(numToAdd, begin = 0) {
                let addSettingPromises = [];
            ...
            const etags = [];
            try {
                for (var _e = true, iterator_1 = __asyncValues(iterator), iterator_1_1; iterator_1_1 = await iterator_1.next(), _a = iterator_1_1.done, !_a; _e = true) {
                    _c = iterator_1_1.value;
                    _e = false;
                    const page = _c;
                    assert.isDefined(page.etag);
                    pageCount++;
                    etags.push((_d = page.etag) !== null && _d !== void 0 ? _d : "");
                }
            }
            catch (e_1_1) { e_1 = { error: e_1_1 }; }
            finally {
                try {
                    if (!_e && !_a && (_b = iterator_1.return)) await _b.call(iterator_1);
                }
            ...
            function assertPage(page, expectedLength, status) {
    ...
    describe("listConfigSettings", function () {
        ...
        afterEach(async () => {
            (await toSortedArray(client.listConfigurationSettings({
        ...
        it("matches any key without label - `backslash0`", async () => {
            ...
            const byLabelSettings = (await toSortedArray(byLabelIterator)).filter((setting) => [key1, key2].includes(setting.key));
    ...
    describe("listRevisions", () => {
        ...
        let originalSetting;
        beforeEach(async () => {
            key = recorder.variable(`listRevisions`, `listRevisions${Math.floor(Math.random() * 100000)}`);
            labelA = recorder.variable(`list-revisions-A`, `list-revisions-A${Math.floor(Math.random() * 100000)}`);
            labelB = recorder.variable(`list-revisions-B`, `list-revisions-B${Math.floor(Math.random() * 100000)}`);
    ...
    describe("setConfigurationSetting", () => {
        it("replaces a configuration setting", async () => {
            ...
            assert.strictEqual(replacedResult.contentType, undefined, "Unexpected contentType in result from setConfigurationSetting().");
            await client.deleteConfigurationSetting({ key, label });
        });
        ...
        it("creates a configuration setting if it doesn't exist", async () => {
            ...
            assert.strictEqual(result.contentType, undefined, "Unexpected contentType in result from setConfigurationSetting().");
        });
        // Skipping all "accepts operation options flaky tests" https://github.com/Azure/azure-sdk-for-js/issues/26447
        it.skip("accepts  operation options", async function () {
            if (isPlaybackMode())
